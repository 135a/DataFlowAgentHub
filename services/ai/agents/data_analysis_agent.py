...
def truncate_data(data: list[dict], max_rows: int = 500) -> list[dict]:
    if len(data) > max_rows:
        logger.warning("Data truncated from %d to %d rows to prevent context overflow.", len(data), max_rows)
...
def data_analysis_node(state: AgentState) -> dict:
    ...
    # ----------------------------------------------------------------
    # 构建 LLM 系统提示（含 RAG 上下文，如有）
    # ----------------------------------------------------------------
    system_prompt = "You are a data analyst. Write a concise business summary of the statistical findings."
