...
扩展限流覆盖范围至认证、数据源管理和用户管理端点，并提供可配置的 fail-closed 降级模式。
...
系统 SHALL 对 `POST /v1/auth/register` 端点应用限流，限制为每用户每分钟 10 次请求。
...
系统 SHALL 提供 `HUB_RATELIMIT_FAIL_CLOSED` 环境变量配置。当设为 `true` 且 Redis 不可用时，系统 MUST 拒绝请求。当设为 `false`（默认）时，保持 fail-open 行为。
